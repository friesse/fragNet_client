...
ClientGC::ClientGC(uint64_t steamId) : m_steamId{steamId} {
...
ClientGC::~ClientGC() { Platform::Print("ClientGC destroyed\n"); }
...
void ClientGC::HandleMessage(uint32_t type, const void *data, uint32_t size) {
  ...
  if (messageRead.IsProtobuf()) {
    ...
    default:
      Platform::Print(
          "ClientGC::HandleMessage: unhandled protobuf message %s\n",
...
void ClientGC::SendMessageToGame(bool sendToGameServer, uint32_t type,
  ...
  const GCMessageWrite &messageWrite =
      m_outgoingMessages.emplace(type, message);
...
constexpr uint32_t MakeAddress(uint32_t v1, uint32_t v2, uint32_t v3,
                               uint32_t v4) {
...
// FROM TCPConnection
void ClientGC::HandleMatchmakingHello(
    const CMsgGC_CC_GC2CL_BuildMatchmakingHello &serverMsg) {
  // store message
  m_serverMatchmakingData = serverMsg;
  m_hasMatchmakingData = true;
  Platform::Print(
      "Received CMsgGC_CC_GC2CL_BuildMatchmakingHello for account %u\n",
  ...
  GCMessageWrite cacheMsg(k_EMsgGC_CC_CL2GC_SOCacheSubscribedRequest,
                          soCacheRequest, true);
...
void ClientGC::StoreCacheSubscription(const CMsgSOCacheSubscribed &message) {
  ...
  Platform::Print(
      "StoreCacheSubscription: Cached %zu items and %zu default equips\n",
      m_items.size(), m_defaultEquips.size());
};

void ClientGC::BuildCacheSubscription(CMsgSOCacheSubscribed &message,
                                      bool server) {
  message = m_serverSOCacheData;

  Platform::Print(
      "BuildCacheSubscription: Using %zu cached items and %zu default equips\n",
      m_items.size(), m_defaultEquips.size());
};
...
void ClientGC::CheckAndSendWelcomeMessages() {
  if (m_hasMatchmakingData && m_hasSOCacheData) {
    ...
    Platform::Print("Both MatchmakingHello and CacheSubscribed received, "
                    "calling SendWelcomeMessages()\n");
    SendWelcomeMessages();
  }
}

void ClientGC::BuildMatchmakingHello(
  ...
  message.set_player_xp_bonus_flags(
      m_serverMatchmakingData.player_xp_bonus_flags());
}

void ClientGC::BuildClientWelcome(
    CMsgClientWelcome &message, const CMsgCStrike15Welcome &csWelcome,
  ...
  message.set_rtime32_gc_welcome_timestamp(
...
void ClientGC::OnClientHello(GCMessageRead &messageRead) {
  ...
  if (!m_hasMatchmakingData) {
    ...
    Platform::Print("Called OnClientHello, now waiting for "
...
void ClientGC::SendWelcomeMessages() {
  ...
  SendMessageToGame(false, k_EMsgGCCStrike15_v2_MatchmakingGC2ClientHello,
                    mmHello);
}

void ClientGC::HandleViewPlayersProfileResponse(
    const CMsgGC_CC_GC2CL_ViewPlayersProfileResponse &message) {
  ...
  for (const auto &profile : message.account_profiles()) {
    ...
    if (profile.has_medals()) {
      ...
      medals->set_featured_display_item_defidx(
          profile.medals().featured_display_item_defidx());
      for (int i = 0; i < profile.medals().display_items_defidx_size(); i++) {
        medals->add_display_items_defidx(
            profile.medals().display_items_defidx(i));
      }
    }
...
void ClientGC::HandleViewPlayersProfileRequest(GCMessageRead &messageRead) {
  ...
  // send it
  if (m_gcConnection && m_gcConnection->CCIsConnected()) {
    GCMessageWrite message(k_EMsgGC_CC_CL2GC_ViewPlayersProfileRequest,
                           viewPlayersProfileRequest, true);
...
// COMMENDS
void ClientGC::HandleClientCommendPlayerQuery(
    GCMessageRead &messageRead) // grrrr
{
  CMsgGCCStrike15_v2_ClientCommendPlayer clientRequest;
  if (!messageRead.ReadProtobuf(clientRequest)) {
    Platform::Print("Parsing CMsgGCCStrike15_v2_ClientCommendPlayerQuery "
                    "failed, ignoring\n");
    return;
  }
  ...
  if (m_gcConnection && m_gcConnection->CCIsConnected()) {
    GCMessageWrite message(k_EMsgGC_CC_CL2GC_ClientCommendPlayerQuery,
                           gcCommendPlayerQuery, true);
    m_gcConnection->NetSendMessage(message);
...
void ClientGC::HandleClientCommendPlayerQueryResponse(
    const CMsgGC_CC_ClientCommendPlayer &message) {
  ...
  if (message.has_commendation()) {
    auto *commendation =
        clientCommendPlayerQueryResponse.mutable_commendation();
...
void ClientGC::HandleClientCommendPlayer(GCMessageRead &messageRead) {
  ...
  if (!messageRead.ReadProtobuf(clientRequest)) {
    Platform::Print(
        "Parsing CMsgGCCStrike15_v2_ClientCommendPlayer failed, ignoring\n");
    return;
  }
  ...
  if (m_gcConnection && m_gcConnection->CCIsConnected()) {
    GCMessageWrite message(k_EMsgGC_CC_CL2GC_ClientCommendPlayer,
                           gcCommendPlayer, true);
    m_gcConnection->NetSendMessage(message);
...
void ClientGC::HandleClientReportPlayer(GCMessageRead &messageRead) {
  ...
  if (!messageRead.ReadProtobuf(clientRequest)) {
    Platform::Print(
        "Parsing CMsgGCCStrike15_v2_ClientReportPlayer failed, ignoring\n");
    return;
  }
  ...
  if (clientRequest.rpt_aimbot())
    reportPlayerRequest.set_rpt_aimbot(1);
  if (clientRequest.rpt_wallhack())
    reportPlayerRequest.set_rpt_wallhack(1);
  if (clientRequest.rpt_speedhack())
    reportPlayerRequest.set_rpt_speedhack(1);
  if (clientRequest.rpt_teamharm())
    reportPlayerRequest.set_rpt_teamharm(1);
  if (clientRequest.rpt_textabuse())
    reportPlayerRequest.set_rpt_textabuse(1);
  if (clientRequest.rpt_voiceabuse())
    reportPlayerRequest.set_rpt_voiceabuse(1);

  // idk what this outputs
  if (clientRequest.has_match_id()) {
    reportPlayerRequest.set_match_id(clientRequest.match_id());
  }
  // reportPlayerRequest.set_report_from_demo(clientRequest.report_from_demo());
  // // apparently not there?

  if (m_gcConnection && m_gcConnection->CCIsConnected()) {
    GCMessageWrite message(k_EMsgGC_CC_CL2GC_ClientReportPlayer,
                           reportPlayerRequest, true);
    m_gcConnection->NetSendMessage(message);
...
void ClientGC::HandleClientReportResponse(
    const CMsgGC_CC_GC2CL_ClientReportResponse &message) {
  ...
  SendMessageToGame(false, k_EMsgGCCStrike15_v2_ClientReportResponse,
                    clientResponse);
}
...
/*void ClientGC::AdjustItemEquippedState(GCMessageRead& messageRead)
{
    ...
    {
        Platform::Print("Parsing CMsgAdjustItemEquippedState failed,
ignoring\n"); return;
    }

    CMsgSOMultipleObjects update;
    if (!m_inventory.EquipItem(message.item_id(), message.new_class(),
message.new_slot(), update))
    {
        // no change
        //assert(false);
        return;
...
void ClientGC::HandleAdjustItemEquippedState(GCMessageRead &messageRead) {
  ...
  if (m_gcConnection && m_gcConnection->CCIsConnected()) {
    GCMessageWrite messageWrite(k_EMsgGC_CC_CL2GC_AdjustItemEquippedState,
                                adjustItemEquippedState, true);
...
void ClientGC::ClientPlayerDecalSign(GCMessageRead &messageRead) {
  ...
  if (!messageRead.ReadProtobuf(message)) {
    Platform::Print(
        "Parsing CMsgGCCStrike15_v2_ClientPlayerDecalSign failed, ignoring\n");
    return;
  }
...
/*void ClientGC::UseItemRequest(GCMessageRead& messageRead)
{
    ...
    if (m_inventory.UseItem(message.item_id(), destroy, updateMultiple))
    {
        ...
        //SendMessageToGame(false, k_EMsgGCItemCustomizationNotification,
notification);
    }
}*/
...
void ClientGC::HandleUseItemRequest(GCMessageRead &messageRead) {
  ...
  for (int i = 0; i < message.gift__potential_targets_size(); i++) {
    useItemRequest.add_gift__potential_targets(
        message.gift__potential_targets(i));
  }
  ...
  if (m_gcConnection && m_gcConnection->CCIsConnected()) {
    GCMessageWrite messageWrite(k_EMsgGC_CC_CL2GC_UseItem, useItemRequest,
                                true);
    m_gcConnection->NetSendMessage(messageWrite);
  }
}

static void AddressString(uint32_t ip, uint32_t port, char *buffer,
                          size_t bufferSize) {
...
void ClientGC::ClientRequestJoinServerData(GCMessageRead &messageRead) {
  ...
  if (!messageRead.ReadProtobuf(request)) {
    Platform::Print("Parsing CMsgGCCStrike15_v2_ClientRequestJoinServerData "
                    "failed, ignoring\n");
    return;
  }
  ...
  AddressString(request.server_ip(), request.server_port(), addressString,
                sizeof(addressString));
  response.mutable_res()->set_server_address(addressString);

  SendMessageToGame(false, k_EMsgGCCStrike15_v2_ClientRequestJoinServerData,
                    response);
}
...
/*void ClientGC::SetItemPositions(GCMessageRead& messageRead)
{
    ...
    if (m_inventory.SetItemPositions(message, acknowledgements, update))
    {
        for (const CMsgItemAcknowledged &acknowledgement : acknowledgements)
        {
            ...
            GCMessageWrite messageWrite{ k_EMsgGCItemAcknowledged,
acknowledgement }; m_networking.NetSendMessage(messageWrite);
        }
...
void ClientGC::HandleSOSingleObject(const CMsgSOSingleObject &message) {
  ...
  if (message.type_id() == SOTypeItem) {
    ...
    if (defaultEquip.ParseFromString(message.object_data())) {
      ...
      Platform::Print("Updated local cache with default equip for account %u, "
                      "class %u, slot %u\n",
                      defaultEquip.account_id(), defaultEquip.class_id(),
                      defaultEquip.slot_id());
    }
  }
...
void ClientGC::HandleSOMultipleObjects(const CMsgSOMultipleObjects &message) {
  ...
  for (int i = 0; i < message.objects_modified_size(); i++) {
    ...
    } else if (message.objects_modified(i).type_id() ==
               SOTypeDefaultEquippedDefinitionInstanceClient) {
      CSOEconDefaultEquippedDefinitionInstanceClient defaultEquip;
      if (defaultEquip.ParseFromString(
              message.objects_modified(i).object_data())) {
        bool found = false;
        for (auto &existingEquip : m_defaultEquips) {
          if (existingEquip.account_id() == defaultEquip.account_id() &&
              existingEquip.class_id() == defaultEquip.class_id() &&
  ...
  for (int i = 0; i < message.objects_added_size(); i++) {
    ...
    } else if (message.objects_added(i).type_id() ==
               SOTypeDefaultEquippedDefinitionInstanceClient) {
      CSOEconDefaultEquippedDefinitionInstanceClient defaultEquip;
      if (defaultEquip.ParseFromString(
              message.objects_added(i).object_data())) {
        m_defaultEquips.push_back(defaultEquip);
  ...
  for (int i = 0; i < message.objects_removed_size(); i++) {
    ...
    } else if (message.objects_removed(i).type_id() ==
               SOTypeDefaultEquippedDefinitionInstanceClient) {
      CSOEconDefaultEquippedDefinitionInstanceClient defaultEquip;
      if (defaultEquip.ParseFromString(
              message.objects_removed(i).object_data())) {
...
void ClientGC::HandleItemAcknowledged(GCMessageRead &messageRead) {
  ...
  for (int startIdx = 0; startIdx < totalItems; startIdx += BATCH_SIZE) {
    ...
    if (m_gcConnection && m_gcConnection->CCIsConnected()) {
      GCMessageWrite message(k_EMsgGC_CC_CL2GC_ItemAcknowledged,
  ...
  Platform::Print(
      "Completed sending acknowledgments for %d items in %d batches to GC\n",
...
/*void ClientGC::IncrementKillCountAttribute(GCMessageRead& messageRead)
{
    ...
    if (!messageRead.ReadProtobuf(message))
    {
        Platform::Print("Parsing CMsgIncrementKillCountAttribute failed,
ignoring\n"); return;
    }
    ...
    if (m_inventory.IncrementKillCountAttribute(message.item_id(),
...
void ClientGC::HandleApplySticker(GCMessageRead &messageRead) {
  ...
  if (m_gcConnection && m_gcConnection->CCIsConnected()) {
    GCMessageWrite messageWrite(k_EMsgGC_CC_CL2GC_ApplySticker,
                                applyStickerRequest, true);
    m_gcConnection->NetSendMessage(messageWrite);
  }
}
...
void ClientGC::HandleStorePurchaseInit(GCMessageRead &messageRead) {
  ...
  if (m_gcConnection && m_gcConnection->CCIsConnected()) {
    ...
    for (int i = 0; i < message.line_items_size(); i++) {
      ...
      lineItem->set_cost_in_local_currency(
          message.line_items(i).cost_in_local_currency());
      lineItem->set_purchase_type(message.line_items(i).purchase_type());
    }

    GCMessageWrite messageWrite(k_EMsgGC_CC_CL2GC_StorePurchaseInit,
                                forwardMessage, true);
    m_gcConnection->NetSendMessage(messageWrite);
...
void ClientGC::HandleStorePurchaseInitResponse(
    const CMsgGC_CC_GC2CL_StorePurchaseInitResponse &message) {
  ...
  SendMessageToGame(false, k_EMsgGCStorePurchaseInitResponse,
                    clientResponse); // this probably isnt enough
...
void ClientGC::HandleUnlockCrate(GCMessageRead &messageRead) {
  ...
  if (m_gcConnection && m_gcConnection->CCIsConnected()) {
    GCMessageWrite message(k_EMsgGC_CC_CL2GC_UnlockCrate, unlockCrateMessage,
                           true);
    m_gcConnection->NetSendMessage(message);
  }
}
...
void ClientGC::HandleCraft(GCMessageRead &messageRead) {
  ...
  if (itemCount != 10) {
    Platform::Print(
        "  Warning: Unexpected item count for trade-up: %u (expected 10)\n",
        itemCount);
  }
  ...
  for (uint16_t i = 0; i < itemCount; i++) {
    if (!messageRead.IsValid()) {
      Platform::Print(
          "  Error: Unexpected end of message while reading item IDs\n");
      return;
    }
...
void ClientGC::HandleCraftResponse(
    const CMsgGC_CC_GC2CL_CraftResponse &gcResponse) {
  ...
  if (gcResponse.response_code() == 0) {
    ...
    if (gcResponse.has_item_object()) {
      const CMsgGC_CC_GC2CL_SOSingleObject &newItemObject =
          gcResponse.item_object();
      ...
      Platform::Print(
          "  Sending new crafted item to game client via k_ESOMsg_Create\n");
...
void ClientGC::HandleNameItem(GCMessageRead &messageRead) {
  ...
  if (m_gcConnection && m_gcConnection->CCIsConnected()) {
    GCMessageWrite messageWrite(k_EMsgGC_CC_CL2GC_NameItem, nameItemRequest,
                                true);
    m_gcConnection->NetSendMessage(messageWrite);
  }
}
...
void ClientGC::HandleNameBaseItem(GCMessageRead &messageRead) {
  ...
  if (m_gcConnection && m_gcConnection->CCIsConnected()) {
    GCMessageWrite messageWrite(k_EMsgGC_CC_CL2GC_NameBaseItem,
                                nameBaseItemRequest, true);
    m_gcConnection->NetSendMessage(messageWrite);
  }
}
...
void ClientGC::HandleRemoveItemName(GCMessageRead &messageRead) {
  ...
  if (m_gcConnection && m_gcConnection->CCIsConnected()) {
    GCMessageWrite messageWrite(k_EMsgGC_CC_CL2GC_RemoveItemName,
                                removeItemNameRequest, true);
    m_gcConnection->NetSendMessage(messageWrite);
  }
}
...
#define HANDLE_MSG(e)                                                          \
  ...
  sprintf(unknown_message, "UNKNOWN MESSAGE %u", type);
