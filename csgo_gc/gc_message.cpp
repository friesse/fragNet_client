...
GCMessageWrite::GCMessageWrite(uint32_t type, const google::protobuf::MessageLite &message, bool customProto)
    // writes a protobuf id depending on if its a custom gc message
    : m_type{ type | (customProto ? CCProtoMask : ProtobufMask) }
{
    // write the protobuf message header
    WriteUint32(m_type);
    WriteUint32(0); // header size always 0

    if (customProto) {
        WriteUint32(1);  // chunk count (1 for now, no splitting yet)
    }
