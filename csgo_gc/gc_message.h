...
public:
    ...
    bool IsCCProto() const { return m_type & CCProtoMask; }
    uint32_t TypeUnmasked() const { return m_type & ~ProtobufMask; }
    uint32_t TypeCCUnmasked() const { return m_type & ~CCProtoMask; }
    ...
    int16_t ReadInt16() { return ReadVariable<int16_t>(); }
...
public:
    // protobuf messages
    GCMessageWrite(uint32_t type, const google::protobuf::MessageLite &message, bool customProto = false);
    ...
    // writing helpers
    void WriteInt16(int16_t value) { WriteData(&value, sizeof(value)); }
